...
#中身はただのpython
  logger = getLogger(__name__)
      data["config"]["rep_macro"] = "rep" data["config"]["long_long_int"] = "ll" data["config"]["indent"] = "  " if not shutil.which("clang-format") :
...
%>
${cplusplus.declare_constants(data)} % if topcoder.is_topcoder(data) :
<%
  solve_function = topcoder.class_name(data) + "()." + topcoder.method_name(data)
%>
class $
{
  topcoder.class_name(data)
} {
  public :
...
% else : <%solve_function = "solve"%> ${cplusplus.return_type(data)} solve(${cplusplus.formal_arguments(data)}){
             // 問題の答え以外の情報を保存したいとき用
             // ofstream outputFile;
             // {
             //   int count = 0;
